...
if __name__ == "__main__":
    ...
    model_action = my_lib.create_model(sample_inputs_len)
    model_comedy = my_lib.create_model(sample_inputs_len)
    model_horror = my_lib.create_model(sample_inputs_len)

    my_lib.train_rosenblatt_linear_model.argtypes = [POINTER(c_float), inputs_type, outputs_type, c_int, c_float, c_int, c_int, c_int]
    my_lib.train_rosenblatt_linear_model.restype = None

    my_lib.train_rosenblatt_linear_model(model_action, inputs_native, outputs_type(*y_train_action), 10000, 0.001, sample_inputs_len+1, inputs_len, outputs_len)
    my_lib.train_rosenblatt_linear_model(model_comedy, inputs_native, outputs_type(*y_train_comedy), 10000, 0.001, sample_inputs_len+1, inputs_len, outputs_len)
    my_lib.train_rosenblatt_linear_model(model_horror, inputs_native, outputs_type(*y_train_horror), 10000, 0.001, sample_inputs_len+1, inputs_len, outputs_len)

    my_lib.predict_linear_model_classification.argtypes = [POINTER(c_float), sample_inputs_type, c_int, c_int]
    my_lib.predict_linear_model_classification.restype = c_float
    predicted_outputs = [[my_lib.predict_linear_model_classification(model_horror, sample_inputs_type(*p), sample_inputs_len+1, sample_inputs_len),
                          my_lib.predict_linear_model_classification(model_action, sample_inputs_type(*p), sample_inputs_len+1, sample_inputs_len),
                          my_lib.predict_linear_model_classification(model_comedy, sample_inputs_type(*p), sample_inputs_len+1, sample_inputs_len)] for p in
                         x_test]
    ...
    my_lib.save_linear_model.argtypes = [POINTER(c_float), c_int, c_char_p]
    my_lib.save_linear_model.restype = None

    my_lib.save_linear_model(model_action, sample_inputs_len+1,  f"test_dataset/linear_model/model_action_linear_{dt_string}.json".encode("utf-8"))
    my_lib.save_linear_model(model_comedy, sample_inputs_len+1,  f"test_dataset/linear_model/model_comedy_linear_{dt_string}.json".encode("utf-8"))
    my_lib.save_linear_model(model_horror, sample_inputs_len+1,  f"test_dataset/linear_model/model_horror_linear_{dt_string}.json".encode("utf-8"))
