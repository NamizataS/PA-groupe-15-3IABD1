...
fn main() {
    let model = create_linear_model(3);
    let dataset_inputs = vec![1.0f64, 4.0, 1.0, -4.0, 4.0, 4.0];
    let dataset_expected_outputs = vec![1.0f64, 1.0, -1.0];
    println!("{:?}",model);
    for _ in 0..10{
        train_rosenblatt_linear_model(&model, &dataset_inputs, &dataset_expected_outputs, 20, 0.001);
    }
    println!("{:?}",model);
}
...
fn train_rosenblatt_linear_model(model: &Vec<f64>, dataset_inputs: &Vec<f64>, dataset_expected_outputs: &Vec<f64>, iterations_count: i32, alpha: f32){
    let mut rng = rand::thread_rng();
    let input_usize = model.len() - 1;
    let input_size = (model.len() - 1) as i32;
